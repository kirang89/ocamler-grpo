...
The repo is script-driven: `train.py`/`train_lora.py` hold the GRPO trainer setup, `generate_solutions_local.py` performs inference plus OCaml evaluation, and helper utilities (conversion, dataset prep) live beside them. Source CSVs (`problems.csv`, `problems.ocaml.csv`) and run artifacts (`metrics.csv`, `grpo_runs/`, `generated_ocaml/`) stay at the top level so experiments remain reproducible. Older workflows are preserved under `backup/`, which still houses `run_ocaml_checks.py` for CSV spot checks.

## Build, Test, and Development Commands
- `uv sync` – install the locked Python 3.13 environment (pip users can fall back to `pip install -e .`).
- `uv run python train.py` – run baseline GRPO training with the current dataset and env-configured model id.
- `uv run python train_lora.py` – same as above but adds the LoRA adapter defined by the `LORA_*` variables.
- `uv run python generate_solutions_local.py` – stream problems from `problems.ocaml.csv`, call Ollama, and emit timestamped CSV plus OCaml files.
...
## Testing Guidelines
Validation hinges on the OCaml toolchain rather than pytest. When touching reward logic, dataset assembly, or subprocess helpers, re-run `uv run python generate_solutions_local.py` and inspect both the console PASS/FAIL summary and the newest `metrics.csv` row. For curated CSVs, `uv run python backup/run_ocaml_checks.py` ingests `blanks.csv`; note any remaining failures inside your PR description.
